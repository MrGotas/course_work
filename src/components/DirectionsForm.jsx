...
import { addDirection, deleteDirection } from "../lib/directions";

export function AddDirectionForm({ setIsCreating }) {
  const router = useRouter();

  const [state, formAction] = useActionState(addDirection, {
    success: false,
    error: "",
  });
...
export const DirectionsList = ({ directions, setIsCreating }) => {
  const router = useRouter();

  return (
    <div className="space-y-4 bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
      <div className="mt-3 flex items-center justify-between text-center sm:mt-0 sm:text-left">
        <h3 className="text-base font-semibold text-gray-900">Направления</h3>
...
export function DirectionsForm({ directions }) {
      ...
      <Dialog open={open} onClose={setOpen} className="relative z-20">
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-start justify-center p-4 text-center sm:p-0">
            ...
            >
              {isCreating ? (
                <AddDirectionForm setIsCreating={setIsCreating} />
              ) : (
                <DirectionsList
